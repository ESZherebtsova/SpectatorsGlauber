...
{    
 ...
 /* fSimTree2->SetBranchAddress("NpartModified", &fNpartModified);
    fSimTree->SetBranchAddress("NspecModified", &fNspecModified);*/
    			}
    if (nEntries < 0){
...
if   (fMode == "Spec") {
/*  hNpartModified = new TH1F ("hNpartModified", "", 700, 0, 700); 
    hNspecModified = new TH1F ("hNspecModified", "", 11000, 0, 11000);*/
    hNpartTarget = new TH1F ("hNpartTarget", "", fNpartMax/fBinSize, 0, fNpartMax );
    hNspecTarget = new TH1F ("hNspecTarget", "", fNcollMax/fBinSize, 0, fNcollMax );
			}
    for (Int_t i=0; i<nEntries; i++)
    {
        ...
        hNpart->Fill(fNpart);
if   (fMode = "Spec") {
	/*hNpartModified->Fill(fNpartModified);
	hNspecModified->Fill(fNspecModified);
	hNpartTarget->Fill(fNpartTarget);
	hNspecTarget->Fill(fNspecTarget);*/
			}
    }
//     fSim->Close();
}
...
if((fMode!="Spec") && (fMode!="SpecModified") && (fMode!="SpecModified2")) {
...
{
	hNspecTarget->SetLineColor(2);
    
        hNspecTarget->Draw();
        hNpartTarget->Draw("same");
        
        TLegend* legSim = new TLegend(0.6,0.75,0.75,0.83); 
        legSim->AddEntry(hNpartTarget ,"NpartTarget", "l");    
        legSim->AddEntry(hNspecTarget ,"NspecTarget", "l");    
        legSim->Draw("same");  
}       
    }
    ...
    if (isData){
        c1->cd(2);
    //  hData->GetYaxis()->SetRangeUser(0., 2e6);    
        hData->Draw();
...
Float_t NpartModified, NspecModified;


 hGlaub = new TH1F("htemp", "", fMultMax/fBinSize, 0, fMultMax);

    hGlaub->SetName(Form("glaub", f, mu, k, n ));
	...
	fSimTree->GetEntry(i);
        ...
        else if  (fMode == "Spec")       Na = f*fNpart + (1-f)*fNspecTarget; 
 /*	else if  (fMode == "SpecModified")       Na = f*fNpartModified + (1-f)*fNspecTarget; 
	else if (fMode=="Spec3") Na=TMath::Power(fNspecTarget, f);*/

   
        Float_t nHits = 0;
	Float_t nHitsTotal = 0;
...
for (Int_t i=fFitMultMin/fBinSize; i<fMultMax/fBinSize; i++)
...
void GlauberFitter::FitGlauber (Int_t nf, Float_t f0, Float_t f1, Int_t nsigma, Float_t SigmaStep, Int_t nEvents)
{
    ...
    TString filename = Form ( "/home/liza/CentralityFramework/CentralityFramework/Glauber/root_files/out.root", dirname.Data(), nf, f0, f1, fFitMultMin );
    TFile *file = new TFile(filename, "recreate");    
    TTree *tree = new TTree("test_tree", "tree" );
    
    TH1F *h1 = new TH1F ("h1", "", 120, 0, 480);
    ...
    for (Int_t i=0; i<nf; i++)
    {
	 ...
	 //  Float_t mu_min = 120, mu_max = 170;

            FindMuGoldenSection (&temp_mu, &temp_chi2, mu_min, mu_max, temp_f, temp_k, nEvents, 5);
...
//                 std::cout << "!!!!!!!!!!!!!!!!!" << std::endl;
    ...
    std::cout << " Total number of events = " << hGlaub->Integral(0, 12000) << std::endl;
    tree->Write ();
    file->Close();
...
Double_t GlauberFitter::GetChi2 ()
{
	...
	//std::cout<<"i = "<<i<<"Glaub = "<<hGlaub->GetBinContent(i)<<"Data = "<<hData->GetBinContent(i)<<"dif = "<<diff<<std::endl;
...
void GlauberFitter::SetNBD2hist(Double_t mu, Double_t k)
{
    // Interface for TH1F.
   /* Int_t nBins2 = 50;
    
    
    hNBD2 = new TH1F("hNBD2","",nBins2, 0, nBins2);
    hNBD2->SetName(Form("nbd_%f_%f",mu,k));
    hNBD2->SetDirectory(0);
    
    for (Int_t i=0; i<nBins2; ++i) 
    {
        Double_t val = NBD(i, mu, k);
        if (val>1e-20) hNBD2->SetBinContent(i+1, val);
